...
// AI-powered daily quiz — 30 questions, 10 categories
// Categories: History, Geography, Polity, Gita, Mahabharata, Ramayana,
//             Culture, Climate, Science, Great Personalities,
//             Sports, Current Affairs, J&K Special
...
const List<Map<String, dynamic>> _fallback = [
  ...
  {'category':'🌟 Great Personalities','question':'Who was the first woman Prime Minister of India?','questionHi':'भारत की पहली महिला प्रधानमंत्री कौन थीं?','options':['Sarojini Naidu','Pratibha Patil','Indira Gandhi','Vijaya Lakshmi Pandit'],'optionsHi':['सरोजिनी नायडू','प्रतिभा पाटिल','इंदिरा गांधी','विजया लक्ष्मी पंडित'],'correctIndex':2,'explanation':'Indira Gandhi became India\'s first (and so far only) woman Prime Minister in 1966. She served three terms.'},
  ...
  {'category':'🏅 Sports','question':'Who is known as the "Flying Sikh" of India?','questionHi':'भारत के "उड़न सिख" के नाम से कौन जाने जाते हैं?','options':['Ajmer Singh','Milkha Singh','Balbir Singh','Gurpreet Singh'],'optionsHi':['अजमेर सिंह','मिल्खा सिंह','बलबीर सिंह','गुरप्रीत सिंह'],'correctIndex':1,'explanation':'Milkha Singh was nicknamed "The Flying Sikh" by Pakistan\'s President Ayub Khan after his record-breaking race in Lahore in 1960.'},
  ...
  {'category':'🏔 J&K Special','question':'What is the traditional handicraft J&K is most famous for?','questionHi':'J&K किस पारंपरिक हस्तशिल्प के लिए सबसे प्रसिद्ध है?','options':['Silk Weaving','Pashmina Shawl','Chikankari','Madhubani Art'],'optionsHi':['रेशम बुनाई','पश्मीना शॉल','चिकनकारी','मधुबनी कला'],'correctIndex':1,'explanation':'Kashmiri Pashmina shawls, made from fine Changthangi goat wool, are world-renowned for their warmth, softness, and intricate embroidery.'},
];
...
13. J&K (Jammu & Kashmir) Special — history, culture, geography, tourism'''
...
{
  "category": "emoji + category (e.g. 🔬 Science)",
  "question": "question in English",
  "questionHi": "question in Hindi (Devanagari script)",
  "options": ["A", "B", "C", "D"],
  "optionsHi": ["A in Hindi", "B in Hindi", "C in Hindi", "D in Hindi"],
  "correctIndex": 0,
...
- Questions must NOT be boring — vary difficulty (easy/medium/hard mix)
- Include interesting, surprising, and motivating facts
...
- For Current Affairs: focus on 2022-2025 events
- For J&K: include history of J&K, famous places, culture, nature, food
- For Great Personalities: include scientists, freedom fighters, sportspersons, artists
''';
    ...
    try {
      ...
      if (res.statusCode == 200) {
        ...
        return parsed.map((q) => _Question.fromJson(q as Map<String, dynamic>)).toList();
      }
    } catch (_) {}
    return [];
  }
...
class _BharatKoJanoScreenState extends State<BharatKoJanoScreen>
  ...
  Future<void> _startQuiz() async {
    setState(() { _loading = true; _loadingMsg = _hindi ? 'AI प्रश्न तैयार कर रहा है…' : 'AI is generating questions…'; });
  ...
  Widget _buildLoading() => Center(child: Column(
    ...
    children: [
      Container(width: 80, height: 80,
        decoration: BoxDecoration(
          gradient: const LinearGradient(colors: [_saffron, _green], begin: Alignment.topLeft, end: Alignment.bottomRight),
          borderRadius: BorderRadius.circular(20)),
        child: const Center(child: Text('🇮🇳', style: TextStyle(fontSize: 40)))),
      const SizedBox(height: 24),
      const SizedBox(width: 36, height: 36, child: CircularProgressIndicator(strokeWidth: 3, color: _green)),
      const SizedBox(height: 16),
      Text(_loadingMsg, style: GoogleFonts.poppins(fontSize: 15, fontWeight: FontWeight.w600, color: AppColors.textPrimary)),
      const SizedBox(height: 6),
      Text(_hindi ? 'AI द्वारा ताज़े प्रश्न • $_questionCount सवाल' : 'AI-powered • $_questionCount questions daily',
  ...
  Widget _buildHome() => SingleChildScrollView(
    ...
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      Container(
        ...
        decoration: BoxDecoration(
          gradient: const LinearGradient(colors: [_navy, Color(0xFF2D5986)], begin: Alignment.topLeft, end: Alignment.bottomRight),
          borderRadius: BorderRadius.circular(22),
          boxShadow: [BoxShadow(color: _navy.withValues(alpha: 0.3), blurRadius: 16, offset: const Offset(0, 6))]),
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [
            ...
            Container(
              ...
              decoration: BoxDecoration(color: _green.withValues(alpha: 0.2), borderRadius: BorderRadius.circular(20),
                border: Border.all(color: _green.withValues(alpha: 0.5))),
              child: Text(_hindi ? 'AI • रोज नए प्रश्न' : 'AI • Fresh Daily',
                  style: GoogleFonts.poppins(fontSize: 10, color: Colors.greenAccent, fontWeight: FontWeight.w700)),
            ),
          ]),
          ...
          return GestureDetector(
            ...
            child: AnimatedContainer(
              ...
              decoration: BoxDecoration(
                ...
                borderRadius: BorderRadius.circular(22),
                border: Border.all(color: sel ? _navy : AppColors.divider, width: sel ? 2 : 1),
                boxShadow: sel ? [BoxShadow(color: _navy.withValues(alpha: 0.25), blurRadius: 8, offset: const Offset(0, 3))] : [],
              ),
      ...
      SizedBox(
        ...
        child: ElevatedButton(
          ...
          style: ElevatedButton.styleFrom(
            backgroundColor: _green, padding: const EdgeInsets.symmetric(vertical: 18),
      ...
      Container(
        ...
        child: Row(children: [
          ...
          Expanded(child: Text(
            _hindi
                ? '$_questionCount प्रश्न, 20 सेकंड प्रति प्रश्न। हर दिन नए प्रश्न!'
  ...
  Widget _buildQuiz() {
    ...
    final timerColor = _timeLeft <= 5 ? const Color(0xFFDC2626)
      ...
      child: ScaleTransition(scale: _scaleAnim, child: Column(
        ...
        children: [
          ...
          Row(children: [
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                ...
                Text('Score: $_score', style: GoogleFonts.poppins(fontSize: 12, color: _green, fontWeight: FontWeight.w700)),
              ]),
              const SizedBox(height: 6),
              ClipRRect(borderRadius: BorderRadius.circular(4),
                child: LinearProgressIndicator(
                  value: progress, backgroundColor: AppColors.divider,
                  valueColor: const AlwaysStoppedAnimation<Color>(_green), minHeight: 7)),
            ])),
            const SizedBox(width: 14),
            AnimatedContainer(
              duration: const Duration(milliseconds: 300),
              width: 54, height: 54,
              decoration: BoxDecoration(color: timerColor.withValues(alpha: 0.12), shape: BoxShape.circle, border: Border.all(color: timerColor, width: 2.5)),
              child: Center(child: Text('$_timeLeft',
                  style: GoogleFonts.poppins(fontSize: 18, fontWeight: FontWeight.w900, color: timerColor))),
            ),
          ]),
          const SizedBox(height: 12),
          ...
          Container(
            ...
            decoration: BoxDecoration(color: _navy.withValues(alpha: 0.08), borderRadius: BorderRadius.circular(20)),
            child: Text(q.category, style: GoogleFonts.poppins(fontSize: 12, fontWeight: FontWeight.w700, color: _navy)),
          ),
          const SizedBox(height: 12),
          ...
          Container(
            ...
            child: Text(question, style: GoogleFonts.poppins(
                fontSize: 16, fontWeight: FontWeight.w700, color: AppColors.textPrimary, height: 1.5)),
          ),
          const SizedBox(height: 14),
          ...
          ...List.generate(options.length, (i) {
            ...
            Color bg = Colors.white, border = AppColors.divider,
                  txt = AppColors.textPrimary, lbl = AppColors.textSecondary;
            Widget? trail;
            ...
            if (_answered) {
              if (isCorrect) {
                ...
                trail = const Icon(Icons.cancel_rounded, color: Color(0xFFDC2626), size: 22);
              } else {
                bg = AppColors.surface; txt = AppColors.textHint; lbl = AppColors.textHint;
              }
            }
              ...
              child: AnimatedContainer(
                ...
                child: Row(children: [
                  Container(width: 32, height: 32,
                    decoration: BoxDecoration(color: lbl.withValues(alpha: 0.12), shape: BoxShape.circle),
                    child: Center(child: Text(String.fromCharCode(65 + i),
                        style: GoogleFonts.poppins(fontSize: 13, fontWeight: FontWeight.w800, color: lbl)))),
                  const SizedBox(width: 12),
                  Expanded(child: Text(options[i], style: GoogleFonts.poppins(fontSize: 14, fontWeight: FontWeight.w600, color: txt))),
                  if (trail != null) trail,
                ]),
              ),
            );
          ...
          if (_answered) ...[
            ...
            Container(
              ...
              child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
                const Icon(Icons.lightbulb_rounded, color: Color(0xFF2563EB), size: 18),
            ...
            SizedBox(width: double.infinity,
              child: ElevatedButton(
                ...
                style: ElevatedButton.styleFrom(
                  backgroundColor: _navy, padding: const EdgeInsets.symmetric(vertical: 15),
  ...
  Widget _buildResults() {
    ...
    final grade = pct >= 90 ? 'A+' : pct >= 80 ? 'A' : pct >= 70 ? 'B+' : pct >= 60 ? 'B' : pct >= 50 ? 'C' : 'D';
    final emoji = pct >= 90 ? '🏆' : pct >= 70 ? '⭐' : pct >= 50 ? '👍' : '📚';
    final msg   = pct >= 90 ? (_hindi ? 'असाधारण! सच्चे भारत ज्ञानी!' : 'Outstanding! True Bharat Gyani!')
                : pct >= 70 ? (_hindi ? 'शाबाश! आप भारत को अच्छी तरह जानते हैं!' : 'Great job! You know India well!')
                : pct >= 50 ? (_hindi ? 'अच्छा प्रयास! सीखते रहिए!' : 'Good effort! Keep learning!')
    ...
    return SingleChildScrollView(
      ...
      child: Column(children: [
        ...
        Container(
          ...
          decoration: BoxDecoration(
            gradient: LinearGradient(colors: [_navy, color], begin: Alignment.topLeft, end: Alignment.bottomRight),
            borderRadius: BorderRadius.circular(22),
            boxShadow: [BoxShadow(color: _navy.withValues(alpha: 0.35), blurRadius: 20, offset: const Offset(0, 8))]),
          child: Column(children: [
            ...
            Text('${pct.toStringAsFixed(1)}%  •  Grade $grade',
                style: GoogleFonts.poppins(fontSize: 16, color: Colors.white70, fontWeight: FontWeight.w600)),
            const SizedBox(height: 10),
        ...
        Row(children: [
          ...
          const SizedBox(width: 10),
          _resStat('❌', '${_questions.length - _score}', _hindi ? 'गलत' : 'Wrong', const Color(0xFFDC2626)),
          const SizedBox(width: 10),
        ...
        Row(children: [
          Expanded(child: OutlinedButton.icon(
            ...
            label: Text(_hindi ? 'होम' : 'Home', style: GoogleFonts.poppins(fontWeight: FontWeight.w700)),
            style: OutlinedButton.styleFrom(
              padding: const EdgeInsets.symmetric(vertical: 15),
              shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(13)),
              side: const BorderSide(color: _navy, width: 2), foregroundColor: _navy),
          )),
          const SizedBox(width: 12),
          Expanded(child: ElevatedButton.icon(
            ...
            style: ElevatedButton.styleFrom(
              backgroundColor: _green, padding: const EdgeInsets.symmetric(vertical: 15),
              shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(13))),
          )),
        ]),
        const SizedBox(height: 12),
